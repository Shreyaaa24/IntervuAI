...
function ResumePanel({
  ...
  recommendedDomains: data.analysis?.recommendedDomains || [],
  experienceLevel: data.analysis?.experienceLevel || "Junior",
  skillsDetected: data.analysis?.skillsDetected || [],
});
      setStep("results");
    } catch (error: any) {
      setError(
        error?.response?.data?.message || "Failed to analyze your resume.",
      );
      setStep("upload");
    } finally {
  ...
  return (
    <Card className="border border-violet-100 bg-white overflow-hidden">
      {/* Header */}
      <div className="px-5 py-4 border-b border-violet-100 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-xl bg-violet-50 border border-violet-100 flex items-center justify-center text-lg">
            📄
          </div>
        ...
        {step === "results" && (
          <button
            onClick={reset}
            className="text-xs text-muted-foreground hover:text-violet-700 border border-violet-100 px-3 py-1 rounded-full transition-colors"
          >
            Upload new ↑
          </button>
        )}
      ...
      <div className="p-5">
        ...
        {step === "upload" && (
          <div className="space-y-4">
              ...
              className={`relative border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all duration-200 ${
                dragging
                  ...
                  : file
                    ...
                    : "border-violet-200 hover:border-violet-400 hover:bg-violet-50/70"
              }`}
            >
              <input
                ref={fileRef}
              ...
              {file ? (
                <div className="flex items-center justify-center gap-3">
                  <div className="w-12 h-12 rounded-xl bg-violet-100 border border-violet-200 flex items-center justify-center text-2xl">
                    📋
                  </div>
            ...
            {!file && (
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div
                    key={item.label}
                    className="flex items-start gap-2 p-3 rounded-xl bg-violet-50/50 border border-violet-100"
                  >
            ...
            <Button
              ...
              disabled={!file}
              className="w-full rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-semibold py-5 disabled:opacity-40 disabled:cursor-not-allowed"
            >
        ...
        {step === "analyzing" && (
          <div className="py-10 flex flex-col items-center gap-6">
            ...
            <div className="w-64 bg-violet-100 rounded-full h-1.5 overflow-hidden">
              <div
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {analysis.skillsDetected?.map((skill) => (
                    <span
                      key={skill}
                      className="text-xs bg-violet-50 text-violet-700 border border-violet-200 px-2.5 py-0.5 rounded-full font-medium"
                    >
                      {skill}
                    </span>
                  ))}
                ...
                {analysis.recommendedDomains?.map((rec, i) => {
                  ...
                  return (
                    <button
                      ...
                      className="w-full flex items-center gap-3 p-3.5 rounded-xl border border-violet-100 hover:border-violet-300 hover:bg-violet-50/60 transition-all group text-left"
                    >
                      <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-violet-50 border border-violet-100 flex items-center justify-center text-xl group-hover:border-violet-300 transition-colors">
                        {meta?.icon || "🎯"}
                      </div>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          {i === 0 && (
                            <span className="text-[10px] bg-violet-600 text-white font-bold px-2 py-0.5 rounded-full">
                              TOP PICK
                            </span>
                          )}
                      ...
                      <span className="text-violet-600 opacity-0 group-hover:opacity-100 transition-opacity text-sm ml-1">
                        →
                      </span>
                    </button>
                  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-violet-50 via-white to-indigo-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 space-y-8">
        ...
        <section className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Button
            ...
            className="bg-violet-600 hover:bg-violet-700 text-white rounded-full px-6 font-semibold shadow-md hover:shadow-lg transition-all self-start sm:self-auto"
          >
            ⚡ New Interview
          </Button>
        </section>
        ...
        {/* Tabs */}
        <section>
          <div className="flex items-center gap-1 mb-6 border-b border-violet-100">
                      ...
                      <button
                        ...
                        className={`text-xs px-3 py-1.5 rounded-full border font-medium transition-all ${
                          filterDomain === d
                            ...
                            : "border-violet-200 text-muted-foreground hover:border-violet-400 hover:bg-violet-50 hover:text-violet-700"
                        }`}
                      >
                        {d}
                      </button>
              ...
              {dataLoading ? (
                <div>
                  {[1, 2, 3].map((i) => (
                    <Card
                      key={i}
                      className="p-5 border border-violet-100 bg-white"
                    >
                      <div className="animate-pulse flex gap-4">
                        <div className="w-10 h-10 rounded-xl bg-violet-100 flex-shrink-0" />

                        <div className="flex-1 space-y-2">
                          <div className="h-4 bg-violet-100 rounded w-1/3" />

                          <div className="h-3 bg-violet-100 rounded w-1/2" />
                        </div>

                        <div className="w-16 h-8 bg-violet-100 rounded-full" />
                      </div>
                    </Card>
                  ))}
                </div>
                ...
                /* Empty State */
                <Card className="p-12 text-center border-2 border-dashed border-violet-200 bg-white">
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 justify-center">
                    <Button
                      onClick={() => setShowDomainSelector(true)}
                      className="bg-violet-600 hover:bg-violet-700 text-white rounded-full px-5 text-sm"
                    >
                      ⚡ Start Interview
                    </Button>
                    ...
                    <Button
                      ...
                      className="rounded-full px-5 text-sm border-violet-200 text-violet-700 hover:bg-violet-50 hover:text-violet-700"
                    >
                      📄 Analyse Resume
                    </Button>
                  </div>
                ...
                <Card className="p-8 text-center border border-violet-100 bg-white">
                ...
                <div>
                  {[...filtered].reverse().map((interview) => {
                    ...
                    return (
                      <Card
                        key={interview.id}
                        className="p-5 border border-violet-100 bg-white hover:border-violet-300 hover:shadow-sm transition-all group"
                      >
                        <div className="flex items-center gap-4">
                          <div className="w-11 h-11 rounded-xl bg-violet-50 border border-violet-100 flex items-center justify-center text-xl flex-shrink-0 group-hover:border-violet-300 transition-colors">
                            {meta?.icon || "🎯"}
                          </div>
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <Button
                              variant="outline"
                              size="sm"
                              className="rounded-full text-xs border-violet-200 text-violet-700 hover:bg-violet-50 hover:text-violet-700 hidden sm:flex"
                            >
                              Details
                            </Button>
                            ...
                            <Button
                              size="sm"
                              className="rounded-full text-xs bg-violet-600 hover:bg-violet-700 text-white"
                              onClick={() =>
      ...
      {ShowDomainSelector && (
        ...
        >
          <Card className="w-full max-w-xl p-6 border border-violet-100 bg-white shadow-2xl">
            <div className="flex items-start justify-between mb-6">
              ...
              <button
                ...
                className="w-8 h-8 rounded-full bg-violet-50 hover:bg-violet-100 flex items-center justify-center text-muted-foreground hover:text-violet-700 transition-colors text-sm"
              >
                ✕
              </button>
            </div>
                ...
                <button
                  ...
                  className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all duration-150 ${
                    hoveredDomain === domain.label
                      ...
                      : "border-violet-100 hover:border-violet-300 hover:bg-violet-50/60"
                  }`}
                >
                  ...
                  <span
                    className={`ml-auto text-violet-600 transition-opacity text-xs ${
                      hoveredDomain === domain.label
                        ? "opacity-100"
                        : "opacity-0"
                    }`}
            ...
            <Button
              variant="ghost"
              className="w-full rounded-xl text-muted-foreground hover:text-violet-700 hover:bg-violet-50 text-sm"
              onClick={() => setShowDomainSelector(false)}
            >
              Cancel
            </Button>
